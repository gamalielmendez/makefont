...
if (process.argv.length === 2) {
    ...
    if (!ValidFlags.includes(process.argv[2])) {
        ...
        let fontfile = join(process.argv[2])
        let enc = "cp1252"
        let embed = true
        let subset = true

        if (!existsSync(fontfile)) {
            console.log("Error: file not found in path: " + fontfile)
            process.exit()
        }
        ...
        if (process.argv.length >= 4) {
            ...
            if (!(enc.toLowerCase() in ValidEncodes)) {
                console.log("Error: Incorrect encoding: " + enc)
                process.exit()
            }
        }
        ...
        console.log('makefont_njs v1.0.8',)
